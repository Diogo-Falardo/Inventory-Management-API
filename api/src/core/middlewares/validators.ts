...
export async function validateBusiness(options: businessValidator) {
  if (!options.userId && !options.businessId) {
    throw Error(
      "Business Validator: Hey developer you need at least an userId or a businessId to use this validator",
    );
  }

  log.withMetadata({ options }).info("validating business");

  if (options.checkUserExists && options.userId) {
    await checkIfUserExistsById(options.userId);
    log.info("user: ok");
  }
